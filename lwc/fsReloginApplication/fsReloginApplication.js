...
export default class FsReloginApplication extends LightningElement {
    ...
    searchAllApplication() {
        ...
        if ((this.appNumber != undefined && this.appNumber != null && this.appNumber != '') || (this.kycNumber != undefined && this.kycNumber != null && this.kycNumber != '')) {
            searchApplication({ appNumber: this.appNumber, kycNumber: this.kycNumber, loanAppNo : '' , recType : '2. Re-login' })
                .then(result => {
    ...
    handleNext(){
        ...
        console.log('Old Applicatio Id ', this.oldAppId);
        cloneExistingApplication({oldApplicationId : this.oldAppId, recTypeName : '2. Re-login'})
        .then(result =>{
