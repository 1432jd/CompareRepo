...
import DOC_DEF_OBJ from '@salesforce/schema/Deferral_Document__c';
...
import STAGE_FIELD from '@salesforce/schema/Deferral_Document__c.Stage__c';
...
import { setDeferralPicklistValue } from './uploadDocumentHelper';
import { stringToDate } from './uploadDocumentHelper';
...
export default class UploadDocument extends NavigationMixin(LightningElement) {
    ...
    @track statusPicklistOptionToShow = [];
    ...
    @track indexNumber = 0
    @track applicationStageOptions = [];

    @wire(getObjectInfo, { objectApiName: DOC_DEF_OBJ })
    deferralObj;
    ...
    @wire(getPicklistValues, { recordTypeId: "$deferralObj.data.defaultRecordTypeId", fieldApiName: STAGE_FIELD })
    getApplicationStage({data, error}){
        //alert(JSON.stringify(data))
        if(data)
        {
            var options = [];
            options.push({label: 'All', value: 'All'})
            data.values.forEach(elm=>{
                options.push({label: elm.label, value: elm.value})
            })
            this.applicationStageOptions = options;
        }
    }
    ...
    handleStageChange(event)
    {
        ...
        this.statusPicklistOptionToShow = [];
        var statisOptions = this.statusPicklistOption;
        var options = [];
        for(var i in statisOptions)
        {
            if(statisOptions[i].value != 'Deferred' && statisOptions[i].value != 'Waived')
            {
                options.push({ label: statisOptions[i].label, value: statisOptions[i].value });
            }
            else if(statisOptions[i].value === 'Deferred' && (this.stageName === 'Disbursal Maker' || this.stageName === 'Disbursal Author'))
            {
                options.push({ label: statisOptions[i].label, value: statisOptions[i].value });
            }
            else if(statisOptions[i].value === 'Waived' && (this.stageName === 'Disbursal Maker' || this.stageName === 'Disbursal Author' || this.stageName === 'Document Deferral'))
            {
                options.push({ label: statisOptions[i].label, value: statisOptions[i].value });
            }
        ...
        this.statusPicklistOptionToShow = options;
    }
    ...
    handleComboboxChange(event) {
        ...
        if (name === 'type-picklist' && (value === 'Application' || value === 'All' || value === 'Mandatory')) {
            ...
            this.isChecked = false;
            this.headerDetails = null;
            this.getDocuments(this.documentType, null, this.stageName);
        }
    ...
    openFullScreen() {
        ...
        if(!this.loaded)
        {
            ...
            this[NavigationMixin.Navigate]({
                ...
                attributes: {
                    //url: '/one/one.app#/n/c__upload_document',
