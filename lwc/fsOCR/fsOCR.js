...
export default class FsOCR extends LightningElement {
    ...
    @track isImg = false;
    @track pdfUrl;
    @track isDirectEntry = false;
    @track submitLabel = 'Submit For OCR';
    @track entryTypeValue = 'OCR';
    ...
    hideDirectEntryForm(event){
        const entrySelect = this.template.querySelector('.entry-type');
        if (entrySelect) {
            entrySelect.value = 'OCR';
            this.entryTypeValue = 'OCR';
        }
        this.isDirectEntry = event.detail;
    }

    hideDirectEntrySave(event){
        this.isPrimary = false;
    ...
    handleCustomerType(event) {
        ...
        this.isPrimary = this.customerTypeValue === 'Primary Applicant' && !this.preloginId ? true : false;
        this.sendCustomerType();
    ...
    handleEntryTypeChange(event){
        ...
        this.isDirectEntry = event.detail.value === 'OCR' ? false : true;
        this.submitLabel = this.isDirectEntry ? 'Submit' : 'Submit For OCR';
        this.hideOcrTable();
    }
    ...
    handleUploadImage(event) {
        ...
        reader.onload = async () => {
            ...
            this.temp64List.push(fileData.base64);
            var count = 0;
            if (this.KYCType != 'Aadhaar Card'){
                this.temp64List.forEach(element => {
                    if(element == fileData.base64)
                        count = count + 1;
                });
                if(count === 1)
            ...
            if (result) {
                ...
                if(result && result.strDataTableData && JSON.parse(result.strDataTableData).length){
                    ...
                    JSON.parse(result.strDataTableData).forEach(currentItem => {
                        currentItem.Id__c = atob(currentItem.Id__c)
                        tempArr.push(currentItem);
                    });
                    result.strDataTableData = JSON.stringify(tempArr);*/
                    this.ocrData = JSON.parse(JSON.stringify(result));
                }
                console.log('getOCRData = ',result);
                if (!this.refreshOCR) {
