...
export default class PcDeviation extends LightningElement {
    ...
    //Added by sangeeta to refresh deviation on tab click
    @api showDev(){
        setTimeout(() => {
            if(this.template.querySelector('c-pc-deviation-table')){
                this.isLoading = true;
                this.devList = false;
                this.template.querySelector('c-pc-deviation-table').getTrRecords();
                this.template.querySelector('c-pc-decisioned-deviation-table').refreshDecisionedDeviationTable();
                this.template.querySelector('c-dviation-approval-screen').refreshDecisionedDeviationTable();
            }
        }, 300); 
    }

    //After getting refresh deviation event

    refreshDev(event){
        console.log('deviation refresh event ',event.detail);
