...
import IS_CHECKER_DONE from '@salesforce/schema/Application__c.Lead_Detail_Checker_Done__c';
...
import checkDecision from '@salesforce/apex/FsLeadDetailsController.checkDecision';
...
export default class FsLeadDetailsChecker extends NavigationMixin(LightningElement) {
    @api recordId;
    @api preLogin;
            ...
            if (result == 'success') {
                checkDecision({ applicationId: this.recordId }).then(result => {
                    ...
                    if (result) {
                        const fields = {};
                        fields[ID_FIELD.fieldApiName] = this.recordId;
                        if (this.stage == 'Awaiting Lead Details Completion') {
                            fields[STAGE_FIELD.fieldApiName] = 'Process Credit';
                        }
                        fields[SUB_FLOW_FIELD.fieldApiName] = '';
                        fields[IS_CHECKER_DONE.fieldApiName] = true;
                        ...
                        this.isSpinnerActive = false;
                        this.showToastMessage('Error', 'Decision Pending!!', 'Error');
                        this.showDecisionTab = true;
                        setTimeout(() => {
                            this.activeError = 'Decision';
                        }, 300);
                    }
                }).catch(err => {
                    console.error(err);
    ...
    async handleSaveSubmit() {
        ...
        await getPendingReasonValidation({applicationId: this.recordId, stage: 'Lead Detail Checker' }).then(result => {
    ...
    finalSubmit() {
        ...
        try {
            ...
            this.template.querySelector('c-fs-lead-details-personal-information-checker').getPersonalInformationData(false, true);
            this.template.querySelector('c-fs-lead-details-employment-details-checker').getEmploymentDetailsAllData(false);
            this.template.querySelector('c-fs-lead-details-property-details-checker').getPropertyVal();
            this.template.querySelector('c-fs-lead-details-property-address-checker').refreshAddNewProperty(false);
            this.template.querySelector('c-fs-lead-details-ownership-details-checker').refreshAddNewProperty(false);
            this.template.querySelector('c-fs-lead-details-loan-type-checker').getLoanTypeData();
    ...
    handleActive(event) {
        ...
        setTimeout(() => {
            if (this.activeError == 'Property Details') {
                this.template.querySelector('c-fs-lead-details-property-details-checker').refreshAddNewProperty();
            }
            if (this.activeError == 'Property Boundaries') {
                this.template.querySelector('c-fs-lead-details-property-boundaries-checker').refreshAddNewProperty();
            }
            if (this.activeError == 'Property Measurement') {
                this.template.querySelector('c-fs-lead-details-property-measurement-checker').refreshAddNewProperty();
            }
            if (this.activeError == 'Property Address') {
                this.template.querySelector('c-fs-lead-details-property-address-checker').refreshAddNewProperty();
            }

            if (this.activeError == 'Ownership Details') {
                this.template.querySelector('c-fs-lead-details-ownership-details-checker').refreshAddNewProperty();
            }
        ...
        try {
            ...
            try {
                ...
                setTimeout(() => {
                    ...
                    this.template.querySelector('c-fs-lead-details-personal-information-checker').getPersonalInformationData(true, false);
                }, "1000")
            } catch (error) {
                console.log(error);
            }
    ...
    handleDecisionSuccess(event) {
        this.showToastMessage('Success', 'Decision Updated Successfully.', 'Success');
    }
    ...
    handleSendBackSubmit(event) {
        ...
        this.updateApplication('Lead Detail Maker');
    }
    ...
    updateApplication(stageValue) {
        ...
        fields[IS_CHECKER_DONE.fieldApiName] = false;
        ...
        updateRecord(recordInput)
            .then((record) => {
                this.isSpinnerActive = false;
                this.showToastMessage('Success', 'Lead Details Checker moved to ' + stageValue + ' successfully.', 'Success');
                window.location.replace('/' + this.recordId);
            })
            .catch(error => {
                this.isSpinnerActive = false;
