...
export default class FsLeadDetailsIncomeDetailsChecker extends LightningElement {
    @api rowAction;
    @api applicationId;
                        ...
                        for (let keyValue of Object.keys(element)) {	
                            if (keyValue != 'Id') {	
                                ...
                                const selectedEvent = new CustomEvent("handletabvaluechange", {	
                                    detail: { tabname: 'Application Information', subtabname: 'Income Details', fieldapiname: keyValue, fieldvalue: element[keyValue], recordId: element.Id }	
                                });	
    ...
    handleSelectedApplication(event){
        ...
        if(event.detail.ActionName === 'view'){
    ...
    //      const selectedEvent = new CustomEvent("handletabvaluechange", {	
    //         detail : {tabname:'Application Information',subtabname:'Income Details',fieldapiname:'Loan_Applicant__c', fieldvalue : this.appIdOnRadioButton, recordId:this.appIdOnRadioButton}	
    //     });	
    ...
    // }	
        ...
        const selectedEvent = new CustomEvent("handletabvaluechange", {	
            detail: { tabname: 'Application Information', subtabname: 'Income Details', fieldapiname: finalFieldAPIName, fieldvalue: tempFieldsContent.CurrentFieldValue, recordId: loandAppId }	
        });	
        // Dispatches the event.	
        this.dispatchEvent(selectedEvent);	

        const selectedEvent2 = new CustomEvent("handletabvaluechange", {	
            detail: { tabname: 'Application Information', subtabname: 'Income Details', fieldapiname: 'Loan_Applicant__c', fieldvalue: this.appIdOnRadioButton, recordId: loandAppId }	
        });	
        // Dispatches the event.	
    ...
    handleSave(){
        ...
        if(data.length > 0){   
         ...
         const removeEvent = new CustomEvent("handletabvalueremove", {	
                detail: { tabname: 'Application Information', subtabname: 'Income Details' }	
            });	
            // Dispatches the event.	
