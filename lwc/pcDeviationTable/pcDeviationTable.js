...
// added for validation on approval authority for same level : 5 dec 22		
import getApprovedDev from '@salesforce/apex/pcDeviationController.getApprovedDev';		
//added for ac user to add deviation from add row : 5 dec 22		
import appIsRaise from '@salesforce/apex/pcDeviationController.appIsRaise';

export default class PcDeviationTable extends NavigationMixin(LightningElement) {
    ...
    @track addRowErrorForBranchUser = true; // if user is not Deviation Approval user for sourcing branch of application this is false means user can not add row
    @track applicantAssetSaved = true;
    //Pending for decision
    //  @track decisionPending= [];
    @track appIsRaiseVar = true; //if true application isdevaition raise is true;
    ...
    @api getTrRecords() {
        ...
        this.getApplicantAset();
        this.applicantAssetSaved = true;
        if (this.isError == false) {
            this.handleHideSave();
        }
        getTrDeviationData({ applicationId: this.applicationId, stageName : this.stageName })
            .then(result => {
                ...
                var refresh = new CustomEvent("getrefreshdev", {
                    detail: (this.data && this.data.length) ? true : false,
                    bubbles: true,
                    composed: true
                });
    ...
    applicableForSelectChange(event) {
        ...
        this.applicantAssetSaved = false;
        if (this.data[rowNo].isNewRow == false) {
            ...
            console.log('this.applicableForUpdate', this.data[rowNo].applicantAsset, this.data[rowNo].isApplication);
    ...
    // get application is deviation raise status so that ac user can add row: 5-Dec 22	
    async appIsRaise(){	
        console.log('app is raised called');	
        await appIsRaise({applicationId : this.applicationId})	
        .then(result =>{	
            console.log('app is deviation raised', result);	
            if(result){	
                this.appIsRaiseVar = true;	
            }	
            else{	
                this.appIsRaiseVar = false;	
            }	
        })	
    }	
     
    async createDeviationRecordRow(event) {
        ...
        console.log('this.addRowErrorForBranchUser', this.addRowErrorForBranchUser);

        // get application is deviation raise status so that ac user can add row: 5-Dec 22	
        //var appIsRaiseVar = true;	
        await this.appIsRaise();	
        var appIsRaiseVar = this.appIsRaiseVar;	
       ...
       /* await appIsRaise({applicationId : this.applicationId})	
        ...
        else if (this.addRowErrorForBranchUser == false && this.appIsRaiseVar == true) {	
            	
            console.log('this.appIsRaiseVar', this.appIsRaiseVar);	
            //this.data[0].devList.Application__r.Is_Deviation_Raised__c == true
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Error',
                    message: 'You are not Deviation Approval User of this Application',
                    variant: 'error'
                })
            );
    ...
    async updateDevCodeValue(event) {
        this.handleShowSave();
        ...
        await this.appIsRaise();	
        if (resultedLevel < 0 && this.appIsRaiseVar == true){
            console.log('level assigned is lower than current deviation level', resultedLevel);
            //if current raised deviation is greater than level of selected deviation than we will heck if current user is Deviation Approval user 
            //for Application Sourcing branch. if not user list of deviation will remain blank and deviation will not raised.
            console.log('this.currentUser', this.currentUser);
            if (this.currentUser.approvalLevel != null && this.currentUser.approvalLevel != undefined) {
                this.data[rowNo].userName = this.currentUser.currentUser.Name;
                this.data[rowNo].devList.Approval_Authority__c = this.currentUser.currentUser.Id;
                this.data[rowNo].devList.Is_Deviation_Raised__c = true;
                this.data[rowNo].iscurrentUser = true;
            }
            else {
                this.data[rowNo].userName = '';
                console.log('this user is not Deviation Approval user for this Application', this.currentUser.approvalLevel);
            }
        }
        else {
    ...
    getCurrentLoginUser() {
        ...
        var currentDeviationLevel = null;
            ...
            .then(result => {
                ...
                if (result != null && result != undefined) {
                    for (var key in result) {
                        console.log('current user is', key, result[key], result[key].currentUser, result[key].approvalLevel);
                        this.currentUser = result[key];
                        this.currentDevLevelToApprove = key;
                        if (result[key].currentUser == undefined || result[key].currentUser == null) {
                            console.log('result[key is null');
                            currentDeviationLevel = key;
                        }
                    }
                    if (currentDeviationLevel != null) {
                        console.log('currentDeviationLevel is null');
                        this.addRowErrorForBranchUser = false;
                    }
                }
                else {
                    // sourcingBranchUser
                    console.log('currentDeviationLevel is null and user');
                    this.addRowErrorForBranchUser = false;
                }
    ...
    sendEmailOption() {
        this.getCurrentLoginUser();
        if (this.currentDevLevelToApprove != undefined && this.currentDevLevelToApprove != null) {
            ...
            //console.log('this.currentUser.approvalLevel', this.currentUser.approvalLevel, this.currentUser.approvalLevel.indexOf('L5'));
            //  if(this.currentUser.length > 0){
            if (this.currentUser.approvalLevel != undefined && this.currentUser.approvalLevel != null && this.currentUser.approvalLevel.indexOf('L5') >= 0) {
                console.log('current deviation level is L5 and login user is', this.currentUser.currentUser, this.data.length);
                for (var devToMail = 0; devToMail < this.data.length; devToMail++) {
                    console.log('loop to check level', this.data[devToMail].devList.Approval_Level__c, this.data[devToMail].devList.Decistion__c, this.currentDevLevelToApprove);
    ...
    handleEmailClick(event) {
        ...
        sendEmailToApprovalUser({ usrToMail: this.data[rowNo].devList.Approval_Authority__c, devDes: this.data[rowNo].devList.Deviation_Description__c, applicationId: this.applicationId, devId: this.data[rowNo].devList.Id })
            .then(result => {
    ...
    async handleSave(event) {
        ...
        //this.isRaise = true;	
        await this.handleNewRowValidation();	
        console.log('this.isError in handle save', this.isError);	
        if (this.isError == false) {	
            this.isRaise = true;
            this.isSave = true;
            for (var dev = 0; dev < this.data.length; dev++) {
                if (this.data[dev].isNewRow == false) {
                    if (this.data[dev].applicantAsset == true) {
                        this.data[dev].devList.Loan_Applicant__c = this.applicableForUpdate.get(this.data[dev].devList.Id);
                    }
                    else if (this.data[dev].applicantAsset == false && this.data[dev].isApplication == false) {
                        this.data[dev].devList.Property__c = this.applicableForUpdate.get(this.data[dev].devList.Id);
                    }
                }
            }
    ...
    async handleNewRowValidation() {
        ...
        if (!this.isError) {
         ...
         // validation added to check approval authority is same for same level	
        //@author : Sangeeta yadav	
        //@date : 17-11-22	
        if (!this.isError) {	
            var levelAndAuthorityMap = new Map();	
            //Get level from Approved and rejected deviation if any: 05-Dec-22	
            console.log('handleNewRowValidation isError for approved deviation', this.isError);	
            await getApprovedDev({applicationId : this.applicationId, stageName : this.stageName})	
            .then(result => {	
                console.log('result from approved deviation', result);	
                for (var key in result) {	
                    console.log('key in approved dev', key, 'result[key]', result[key]);	
                    levelAndAuthorityMap.set(key, result[key]);	
                }	
            })	
            .catch(error => {	
                console.log('result from approved deviation', error);	
            })	
            console.log('Map from approved or rejected deviations', levelAndAuthorityMap);	
            for (var allDev = 0; allDev < this.data.length; allDev++) {	
                console.log('devList.Approval_Level__c', )	
                let approvalLevel = this.data[allDev].devList.Approval_Level__c;	
                let authority = this.data[allDev].devList.Approval_Authority__c;	
                console.log('approvalLevel', approvalLevel, 'authority', this.data[allDev].devList.Approval_Authority__c);	
                //level is already maped to an user	
                if(levelAndAuthorityMap != null && levelAndAuthorityMap.size > 0 && this.data[allDev].iscurrentUser == false){	
                    if(authority == null || authority == '' || authority == undefined){	
                        console.log('authority null no error for authority level');	
                    }	
                    else if(authority != null || authority != '' || authority != undefined){	
                        if(levelAndAuthorityMap.has(approvalLevel) && levelAndAuthorityMap.get(approvalLevel) == authority){	
                            continue;	
                        }	
                        else{	
                            if(levelAndAuthorityMap.has(approvalLevel)){	
                                this.isError = true;	
                                // add validation Approval authority for all deviation for this level should same	
                                this.dispatchEvent(	
                                    new ShowToastEvent({	
                                        title: 'Error',	
                                        message: 'Please select same Approval Authority for respective Approval level.',	
                                        variant: 'error'	
                                    })	
                                );	
                                //this.isRaise = false;	
                                ...
                                levelAndAuthorityMap.set(approvalLevel, authority);	
                            }	
                        }	
                    }	
                }	
                ...
                if(this.isError){	
                    console.log('handleNewRowValidation isError', this.isError);	
                    /*this.dispatchEvent(	
                        new ShowToastEvent({	
                            title: 'Error',	
                            message: 'Please select same Approval Authority for respective Approval level.',	
    ...
    // Added on 05/11/22 to delete deviation not saved
    //handle delete
    handleDelete(event) {
        ...
        let rowNo = parseInt(event.target.getAttribute("data-row-index"));
        let tempData = JSON.parse(JSON.stringify(this.data));
        this.data = [];
        console.log('rowNo', rowNo);
        if (tempData[rowNo].isNewRow == true) {

            //this.data.splice(rowNo, 1);

            this.isRaise = true;
            this.deviationSpinner = true;
            // console.log('this.data before splice', this.data.length);
            console.log('tempData before splice', JSON.parse(JSON.stringify(tempData)));
            console.log('row to delete', rowNo);
            tempData.splice(rowNo, 1);
            console.log('tempData after splice', JSON.parse(JSON.stringify(tempData)));
            console.log('number of rows after delete', tempData.length);
            let row = 1;

            let tempArr = JSON.parse(JSON.stringify(tempData))
            tempArr.forEach(currentItem => {
                currentItem.rowNumber = row;
                console.log('currentItem.rowNumber', currentItem.rowNumber);
                row++;
            });
            console.log('JSON.parse(JSON.stringify(tempData))', JSON.parse(JSON.stringify(tempArr)));
