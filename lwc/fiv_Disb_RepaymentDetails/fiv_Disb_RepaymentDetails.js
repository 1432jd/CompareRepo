...
export default class Fiv_Disb_RepaymentDetails extends LightningElement {
    ...
    handleChildSlcdBankDetail(event) {
        ...
        if (this.mapRepayTypeWithId.size < this.maxNumOfRepaymentrecs) {
            ...
            try {
                ...
                //objRepayment.NACH_IFSC_Code__c = event.detail.recordData['IFSC_Code__c'];
                objRepayment.NACH_IFSC_Code__c = event.detail.recordData.IFSC_Code__c;
                objRepayment["Application__c"] = this.obj_parent_appt_wrapper_data.objAppt.Id;//Karan SIngh : 05-10-2022 : CH
                var newMask = '';
            ...
            this.showNotification('', 'Only one NACH records allowed per applicant.', 'warning'); //incase if any apex exception happened it will show notification
        }
    ...
    getExistRepayRecords(slcdApplicantName) {
        ...
        console.log('this.existDisbursalId in method ' , this.existDisbursalId);
        this.mapRepayTypeWithId = new Map();
    if(this.existDisbursalId){
        getParentAndRelatedData({
            ...
            this.showNotification('ERROR', 'Please complete Disbursal Parameter  first', 'error');
                return;
        }
    }
