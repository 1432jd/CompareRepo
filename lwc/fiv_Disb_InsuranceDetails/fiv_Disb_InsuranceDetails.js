...
export default class Fiv_Disb_InsuranceDetails extends NavigationMixin(LightningElement) {
    ...
    //@track loanAppId; //New Changes
    @track insuredPersonId;
        ...
        getLoanAplicantNames({ applicationId: this.applicationId }).then(result => {
            if (result) {
                result.forEach(currentItem => {
                    this.loanApplicantNames.push({ label: '' + currentItem.split('_')[0], value: '' + currentItem });
                });
            }
        })
        ...
        getNominee({ applicationId: this.applicationId })
            .then(result => {
                ...
                if (this.applicationRecords != null) {
                    this.insuredId = this.applicationRecords.Insured_Person__c;
                    ...
                    this.insuranceDetailRecord.Insured_Person_Name__c = this.applicationRecords.Insured_Person_Name__c + '_' + this.applicationRecords.Insured_Person__c;
                    this.insuranceDetailRecord.Nominee_Name__c = this.applicationRecords.Nominee_Name__c + '_' + this.applicationRecords.NomineeName__c;
                    ...
                    console.log('this.trackInsurance PG FIRST ', this.trackInsurance);
                }
    ...
    getLoanApptNames() {
        ...
        getLoanAplicantNames({ applicationId: this.applicationId }).then(result => {
            ...
            try {
                for (let index in result) {
                    console.log('name ', index, result[index].split('_')[0]);
                    this.loanApplicantNames.push({
                        label: '' + result[index].split('_')[0], value: '' + result[index]
                    });
                }
            } catch (error) {
    ...
    reInitInsuranceDetail() {
        ...
        if (this.obj_parent_appt_wrapper_data.objAppt.hasOwnProperty('Name__c')) {
            ...
            this.insuranceDetailRecord.Insured_Person_Name__c = this.obj_parent_appt_wrapper_data.objAppt.Name__c + '_' + this.insuredId;
        }
        this.getfeeData();
    ...
    handleInputChange(event) {
        ...
        if (event.target.dataset.field == 'Insured_Person_Name__c') {
            console.log('event.detail.value Pg ', event.detail.value);
            this.insuredPersonId = event.detail.value.split('_')[1];
            console.log('this.insuredPersonId ', this.insuredPersonId);
            if (this.objinsurance) {
                this.objinsurance[0].InsuredPersonName = event.detail.value.split('_')[0];
            }
            console.log('Applicant DOB', this.applicantDOBs);
            this.insuranceDetailRecord.Insured_Person_Date_of_Birth__c = this.applicantDOBs[event.detail.value.split('_')[0]];
            this.insuranceDetailRecord.Insured_Person_Name__c = event.detail.value.split('_')[0];
            this.insuranceDetailRecord.Insured_Person__c = this.insuredPersonId;
            console.log('this.insuranceDetailRecord pg ', this.insuranceDetailRecord);
        }
        ...
        if (event.target.dataset.field == 'Nominee_Name__c' /*&& this.nomineePartOfLoan*/) {
            if (event.target.dataset.type === 'combobox') {
                ...
                console.log('this.nomineePersonId ', this.nomineePersonId);
                this.insuranceDetailRecord.NomineeName__c = this.nomineePersonId;
                this.insuranceDetailRecord.Nominee_Name__c = event.detail.value.split('_')[0];
            }
            if (this.objinsurance) {
                if (event.target.dataset.type === 'combobox') {
                    this.objinsurance[0].NomineeName = event.detail.value.split('_')[0];
                }
                else {
            ...
            var applicantName = '';
            if (event.target.dataset.type === 'combobox') {
                applicantName = event.detail.value.split('_')[0];
            }
            else {
                applicantName = event.detail.value;
            }
    ...
    enableDisableFields() {
        ...
        try {
            ...
            console.log('this.insuranceDetailRecord pg 1 ', this.insuranceDetailRecord);
    ...
    handleSave() {
        ...
        if(this.insuranceDetailRecord.Insured_Person_Name__c.includes('_'))
            this.insuranceDetailRecord.Insured_Person_Name__c = this.insuranceDetailRecord.Insured_Person_Name__c.split('_')[0];
        if(this.insuranceDetailRecord.Nominee_Name__c.includes('_'))
            this.insuranceDetailRecord.Nominee_Name__c = this.insuranceDetailRecord.Nominee_Name__c.split('_')[0];
        ...
        if (isDataValid || this.insuranceDetailRecord.Insurance_Waiver__c == 'Yes') {
            ...
            if (this.insuranceDetailRecord.Insurance_Waiver__c == 'Yes') {
                ...
                this.insuranceDetailRecord.Insured_Person__c = null;
                ...
                this.insuranceDetailRecord.NomineeName__c = null;
    ...
    // handleSelectedInsName(event){
    //     console.log('loanAppId ',event);
    //      if (event.detail.length > 0) {
    //         this.loanAppId = event.detail[0].id;
    //     } else {
