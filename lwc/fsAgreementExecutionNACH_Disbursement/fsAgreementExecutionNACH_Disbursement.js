...
export default class FsAgreementExecutionNACH_Disbursement extends NavigationMixin(LightningElement) {
    ...
    application({ error, data }) {
        console.log('applicationDetailsStage = ', data);
        if (data) {
            this.stage = data.fields.Stage__c.value;
            if (this.stage === 'Disbursal Author') {
                 setTimeout(() => {
                    this.rowAction.splice(0,1);
                 }, 200);
            }
        } else if (error) {
    ...
    handleSelectedBank(event) {
        ...
        if (event.detail.ActionName === 'verify') {
            ...
            callPennyDropAPI({ BankDetailId: this.bankDetailId }).then(result => {
                this.getBankRecordDetails();
                this.isSpinnerActive = false;
                this.showToast(result.split("_")[0], result.split("_")[0], result.split("_")[1]);
            })
            ...
            callEmandateBankListAPI({ BankDetailId: bankId })
                .then(result => {
                    ...
                    if (result == true) {
                        this.isSpinnerActive = true;
                        this.isNachVerification = false;
                        console.log('Inside bank verification',this.eNachRowAction.length);
                        setTimeout(() => {
    ...
    async handleValueChange(event) {
        ...
        if (this.isRecordEdit) {
            if (details.CurrentFieldAPIName === 'Bank_Detail__c-IFSC_Code__c' || details.CurrentFieldAPIName === 'Bank_Detail__c-Account_Number__c') {
                console.log('Yes i changed');
                this.isBankDetailChanged = true;
            }
        }
        /*if (details.CurrentFieldAPIName === 'Bank_Detail__c-IFSC_Code__c' && details.CurrentFieldValue !== true) {
        ...
        } else if (details.CurrentFieldAPIName === 'Bank_Detail__c-IFSC_Code__c') {
    ...
    enterpressed(event) {
        ...
        console.log('ifscCode ', typeof ifscCode);
        try {
                ...
                getIFSCData({ ifscCode: ifscCode })
                    .then(data => {
                        ...
                        if (data.isSuccess) {
                            ...
                            this.showToast('Error', 'Error', 'Enter A Valid IFSC Code.');
        ...
        if (data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                ...
                await saveRecord({ dataToInsert: JSON.stringify(data[i]) })
                    .then(result => {
                        console.log('bank detail save result ', result);
                        this.fieldsContent = {};
                        this.isSpinnerActive = false;
                        var message;
