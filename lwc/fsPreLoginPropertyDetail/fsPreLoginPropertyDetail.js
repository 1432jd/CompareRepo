...
export default class FsPreLoginPropertyDetail extends LightningElement {
    ...
    getPropertyObjectData({ data, error }) {
        if (data) {
            ...
            this.propertyRecordTypeId = Object.keys(recordTypeData).find(rti => recordTypeData[rti].name === 'Pre Login Property Detail');
        }
    }
    ...
    @api getIdsOnEdit(propWrap) {
        ...
        this.natureOfProperty = propWrap.NatureOfProp;
        this.typeofProperty = propWrap.PropertyType;
    ...
    async handleSave() {
        ...
        if (!this.natureOfProperty) {
            this.showToast('Error', 'Error', 'Complete Required Field,Nature Of Property');
            return;
        }
        if (!this.typeofProperty) {
            this.showToast('Error', 'Error', 'Complete Required Field,Type Of Property');
            return;
        }
        ...
        if (!titleDeedExist) {
            ...
            if (data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    data[i].Property_Address__c = data[i].Address_Line_2__c;
                    if(data[i].Address_Line_3__c!= null)
                        data[i].Property_Address__c += ','+data[i].Address_Line_3__c;
                                    ...
                                    createPropertyOwners({ propertyId: this.propertyId, loanAppList: this.propOwnerList })
                                        .then(dataresult => {
                                            if (dataresult) {
                                                ...
                                                this.showToast('Error', 'Error', 'Unable to save record.');
                                                this.closeAction();
                                            }
                                        })
                                        .catch(error => {
