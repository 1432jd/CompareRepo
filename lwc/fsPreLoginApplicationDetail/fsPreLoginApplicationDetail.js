...
import OLD_APP_NUMBER_FIELD from '@salesforce/schema/Application__c.Old_Application_Number__c';
...
export default class FsPreLoginApplicationDetail extends LightningElement {
    ...
    @wire(getRecord, { recordId: '$applicationId', fields: [OLD_APP_NUMBER_FIELD] })
    oldAppDetails({ error, data }) {
        console.log('oldAppDetails data ',data);
        if (data) {
            if(!this.oldAppNumber)
                this.oldAppNumber = data.fields.Old_Application_Number__c.value;
        } else if (error) {
            console.error(error);
            //this.error = error;
        }
    ...
    handleFormValueChange(event) {
        ...
        else if(tempFieldsContent.CurrentFieldAPIName === 'Application__c-Sourcing_Branch__c'){
            if(tempFieldsContent.CurrentFieldValue){
                this.setValues('Sourcing_Branch__c',tempFieldsContent.CurrentFieldValue)
            }
        }
        ...
        for(var i=0; i<_tempVar[0].fieldsContent.length; i++){
            if(_tempVar[0].fieldsContent[i].fieldAPIName === _fieldAPIName){
                if(_tempVar[0].fieldsContent[i].isCheckbox){
                    ...
                    else if(_tempVar[0].fieldsContent[i].fieldAPIName === 'Sourcing_Branch__c'){
                        _tempVar[0].fieldsContent[i].lookupVal = _val;
                    }
    ...
    async handleSave(){
        ...
        if(!this.fieldOfficerEMPId){
            this.showToast('Error','Error','Missing Field Sourcing Officer!!');
            return;
        }
        ...
        if(data.length > 0){ 
            ...
            for(var i=0; i<data.length; i++){
                ...
                if(this.fieldOfficerEMPId)
                    data[i].Sourcing_Officer__c = this.fieldOfficerEMPId;
