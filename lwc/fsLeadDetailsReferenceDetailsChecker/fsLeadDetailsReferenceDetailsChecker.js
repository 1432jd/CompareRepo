...
export default class FsLeadDetailsReferenceDetailsChecker extends LightningElement {
    ...
    getSectionPageContent(recId){
        ...
        getSectionContent({recordIds : this.recordId, metaDetaName : 'Lead_Details_Reference_Details_Checker'})
        .then(result => {
    ...
    changedFromChild(event){
        ...
        const selectedEvent = new CustomEvent("handletabvaluechange", {	
            detail: { tabname: 'Application Information', subtabname: 'Reference Details', fieldapiname: finalFieldAPIName, fieldvalue: tempFieldsContent.CurrentFieldValue, recordId: this.recordId }	
        });	
    ...
    setValues(_fieldAPIName,_val){
        ...
        this.fieldsContent = JSON.stringify(_tempVar);
            ...
            for (var i = 0; i < _tempVar[0].fieldsContent.length; i++) {	
                if (_tempVar[0].fieldsContent[i].fieldAPIName === 'Refered_By_Introducer_Broker_Name__c' || _tempVar[0].fieldsContent[i].fieldAPIName === 'Refered_By_Customer_Loan_Account_No__c') {	
                    ...
                    const selectedEvent = new CustomEvent("handletabvaluechange", {	
                        detail: { tabname: 'Application Information', subtabname: 'Reference Details', fieldapiname: fName, fieldvalue: fValue, recordId: rcId }	
                    });	
                    this.dispatchEvent(selectedEvent);	
                }	
            }	
